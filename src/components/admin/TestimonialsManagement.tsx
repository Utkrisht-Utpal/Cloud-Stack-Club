...
export const TestimonialsManagement: React.FC = () => {
  ...
  const parsedEditOrder = typeof editDisplayOrder === 'number' ? editDisplayOrder : parseInt(String(editDisplayOrder), 10);
  const existingWithSameEditOrder = editingTestimonial && !isNaN(parsedEditOrder) && parsedEditOrder > 0
    ? testimonials.find((t) => t.id !== editingTestimonial.id && t.display_order === parsedEditOrder)
    : undefined;
  ...
  return (
    <div className="p-4 sm:p-6 rounded-3xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 shadow-xl space-y-6">
                    ...
                    <div className="flex items-start justify-between gap-3 mb-2">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          className="p-1.5 rounded-lg text-slate-400 hover:text-blue-600 dark:hover:text-sky-400 hover:bg-blue-500/10 transition-colors cursor-pointer"
                        >
          ...
          <form onSubmit={handleSaveEdit} className="space-y-4 pt-1">
            ...
            <div>
              ...
              <textarea
                ...
                required
                placeholder="Enter what the participant said about the event..."
                className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-300 dark:border-slate-700 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors resize-none"
              />
            </div>

            {/* Event and Order Selection */}
            <div className="grid grid-cols-12 gap-3 items-end">
              <div className="col-span-8">
                ...
                <CustomSelect
                  options={eventSelectOptions}
                  value={editSelectedEventName}
                  onChange={(val) => setEditSelectedEventName(val)}
                  placeholder="Select Event from Database"
                  leadingIcon={<Calendar className="w-4 h-4 text-blue-500" />}
                />
              </div>
              ...
              <div className="col-span-4">
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => {
                      const val = e.target.value;
                      if (val === '') setEditDisplayOrder('');
                      else setEditDisplayOrder(Math.max(1, parseInt(val, 10) || 1));
                    }}
                    className={`w-full h-[42px] pl-3 pr-8 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/80 border text-sm font-bold text-slate-900 dark:text-slate-100 focus:outline-none transition-colors ${
                      existingWithSameEditOrder
                        ? 'border-amber-500 ring-1 ring-amber-500/50'
                        : 'border-slate-300 dark:border-slate-700 focus:border-blue-500 focus:ring-1 focus:ring-blue-500'
                    }`}
                    required
            ...
            {/* Live Duplicate Warning with 1-Click Fix */}
            {existingWithSameEditOrder && (
              <div className="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-between gap-2 text-amber-600 dark:text-amber-400 text-xs">
                <div className="flex items-center gap-1.5 min-w-0">
                  <AlertCircle className="w-4 h-4 shrink-0" />
                  <span className="truncate">
                    Order #{parsedEditOrder} is taken by <strong>{existingWithSameEditOrder.author_name}</strong>
                  </span>
                </div>
                <button
                  type="button"
                  onClick={() => setEditDisplayOrder(nextAvailableOrder)}
                  className="px-2 py-0.5 rounded-md bg-amber-500/20 hover:bg-amber-500/30 font-bold shrink-0 text-[11px] transition-colors cursor-pointer"
                >
                  Use #{nextAvailableOrder}
                </button>
            ...
            {editSelectedEventName === '__custom__' && (
              <div>
                <input
                  type="text"
                  value={editCustomEventName}
                  onChange={(e) => setEditCustomEventName(e.target.value)}
                  placeholder="Type custom or past event name..."
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-blue-500/50 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  required
                />
              </div>
            )}
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="e.g. Rahul Sharma"
                  className="w-full pl-3.5 pr-10 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-300 dark:border-slate-700 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors"
                  required
                />
            ...
            <div>
              ...
              <div className="relative">
                <input
                  type="text"
                  value={editAuthorPosition}
                  onChange={(e) => setEditAuthorPosition(e.target.value)}
                  placeholder="e.g. CSE - 3rd Year / Full Stack Lead"
                  className="w-full pl-3.5 pr-10 py-2.5 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-300 dark:border-slate-700 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors"
                />
                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-slate-400">
      ...
      <ConfirmModal
        ...
        title="Delete Testimonial"
        message="Are you sure you want to permanently delete this testimonial? It will be removed from the Supabase database, Admin Panel, and the public Main Page."
